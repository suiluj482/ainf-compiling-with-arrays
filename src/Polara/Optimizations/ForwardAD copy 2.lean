...
-- #eval EnvDf.ty Ty.flt [⟨Ty.flt, (.v (.mk 1))⟩, ⟨Ty.lin, (.v (.mk 1))⟩]

@[reducible]
def Ty.dfEnv (env: EnvDf): Ty → Ty
| α => (α.df ×× (env.ty α.df'.linRet))

def EnvDf.wrap (env: EnvDf)(m: ListMap VPar (Γ ·.df'.linArg))(a: ListMap VPar (Γ ·.df'.linArg) → Tm Γ α): Tm Γ (env.ty α) :=
  match env with
  | [] => a m
  | ⟨β, x⟩ :: env' => fun'v v => (EnvDf.wrap env' (⟨β,x,v⟩  :: m) a)

def EnvDf.unwrap (env: EnvDf)(m: ListMap VPar (VPar ·.df'.linArg))(a: Tm VPar (env.ty α)): Tm VPar α:=
  match env with
  | [] => a
  | ⟨_, x⟩ :: env' => EnvDf.unwrap env' m (a @@ (Tm.var (m.lookup x).get!))

...
@[reducible]
private def EnvDf := List (Sigma VPar)
@[reducible]
...
private def linOpDf [t: BiArraysC BiLin α β γ](op: AddOp)
  (a: Tm Γ α.df)(b: Tm Γ β.df): Tm Γ γ.df :=
  match t.t with
  | .array n t' =>
...
private def linScaleDf [t: BiArrayC BiLF α β γ](op: MulOp)
  (a: Tm Γ α.df)(b: Tm Γ β.df): Tm Γ γ.df :=
...
private def Const2.df (a: Tm Γ α.df)(b: Tm Γ β.df): Const2 α β γ → Tm Γ γ.df
...
private def VPar.dfEnv (env: EnvDf): VPar α → VPar (α.dfEnv env) := VPar.changeType
private def VPar.idfEnv (env: EnvDf): VPar (α.dfEnv env) → VPar α := VPar.changeType

private def VPar.df: VPar α → VPar α.df := VPar.changeType
          ...
          let rec go (env): Term α.df × Term (env.ty α.df'.linRet) :=
            if env.length ≤ depth
              then let t := Tm.var (v.dfEnv env); (t.fst, t.snd)
                ...
                | [] => panic! "Tm.df' Tm.var depth in ren cant be right"
                | _ :: env => go env |>.map id (fun'v _ => ·)
          let (v, d) := go env; (v,,d)
      | none =>

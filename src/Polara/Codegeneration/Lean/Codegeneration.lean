...
| Const2.arithOp op => s!"{a} {op} {b}"
| Const2.linOp op => s!"{a} {op} {b}"
| Const2.linScale op => s!"{a} {op} {b}"
| .lt => s!"Bool.toNat ({a} < {b})"
| .maxf => s!"max {a} {b}"
| .addi => s!"{a}.add' {b}"
...
| .zipL => s!"List.zip {a} {b}"
| .mapL => s!"←{a}.mapM {b}"
| .foldL (β:=β) => s!"←{a}.foldlM (λ acc a => do (((←({b}.fst a)) acc): {β.gen})) {b}.snd"
| .foldA (β:=β) => s!"←{a}.foldlM (λ acc a => do (((←({b}.fst a)) acc): {β.gen})) {b}.snd"
...
| abs (α:=γ) (β:=β) f => do
  let (i,j) <- read
  let v := VPar.p (.mk j)
  return s!"(return fun {v}:{γ.gen'} => do (({(f v).codegen' (i,j+1) |>.indent}): {β.gen}))"
| bld (n:=n) (α:=β) f => do
  let (i,j) <- read
  let v := VPar.p (.mk j)
...
| bnd (α:=β) e f => do
  let (i,j) <- read
  let x := VPar.v (.mk i)
  return s!"\nlet {x}: {β.gen} := (do {e.codegen' (i,j)}); {(f x).codegen' (i+1,j) |>.ensureLineBreak}"
| ite cond a b =>
  return s!"(if ((←{← cond.codegen'}) != 0) {s!"then (do {(<- a.codegen').indent}".indent}){s!"else (do {(<- b.codegen').indent}".indent}))"
